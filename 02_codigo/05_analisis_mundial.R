...
novel_x_pais %>%
  ...
  coord_flip() +
  labs(title = "Estatus de pacientes que enfermaron por el Covid-19 en cada país\ny territorio",
       subtitle = subtitulo_treemaps,
       x = NULL,
       y = "\nPorcentaje                       ",
       fill = NULL,
...
novel_x_pais %>% 
  ...
  geom_col(fill = "black") +
  scale_y_continuous(limits = c(0, 12.8),
                     expand = c(0, 0)) +
  coord_flip() +
...
datos %>% 
  ...
  labs(title = "Número diario de muertes de pacientes enfermos de Covid-19",
       subtitle = subtitulo_treemaps,
       x = "",
       y = "Número\n",
       caption = "\nElaborado por @segasi para el Buró de Investigación de ADN40 / Fuente: CSSE de la Universidad Johns Hopkins") +
...
datos %>% 
  ...
  scale_y_continuous(labels = comma, 
                     ...
                     expand = c(0, 0)) +
  labs(title = "Número diario de pacientes que se recuperaron de Covid-19",
       subtitle = subtitulo_treemaps,
       x = "",
       y = "Número\n",
       caption = "\nElaborado por @segasi para el Buró de Investigación de ADN40 / Fuente: CSSE de la Universidad Johns Hopkins") +
...
faa %>% 
  ...
  geom_point(aes(x = dias_primer_caso, 
             ...
             size = 2) +
  geom_text(aes(label = etiquetas_paises_casos), 
            vjust = -0.8, 
            color = "grey30",
            fontface = "bold", 
            size = 5) +
...
faa %>% 
  ...
  geom_point(aes(x = dias_primer_caso, 
             ...
             size = 2) +
  geom_text(aes(label = etiquetas_paises_muertes), 
            vjust = -0.8, 
            color = "grey30", 
            fontface = "bold", 
            size = 5) +
