...
datos <- 
  ...
  mutate(casos_confirmados = ifelse(fecha_corte == max(fecha_corte) & pais == "México", 367, casos_confirmados),
         muertes = ifelse(fecha_corte == max(fecha_corte) & pais == "México", 4, muertes),
         recuperados = ifelse(fecha_corte == max(fecha_corte) & pais == "México", 4, recuperados),
         total_casos = ifelse(pais == "México", 367, total_casos), 
